...
// NOTE:  Different from TemperatureSimulation demo, in that the room number
// is used as the key instead of the id number
var rooms = {
		"-1": {
			"name": "202", 
...
// Original picture Scaife floor 2 dimensions 648w x 1147h
...
function drawGradient(svg) {
	...
	if (showRoomInfoGroup) {
		gradientPadding = 8*labelPadding;
	} 
...
function drawRoomInfo() {
	...
	roomInfoGroup = svg.append("g")
			.attr("transform", "translate(" + 
		 			(imageWidth + 3*labelPadding) + "," + 2*labelPadding + ")");
...
function rgb(temp) {
	// If temperature is 0, color grey
	// This indicates there is a sensor installed in the room, but it is not 
	// sending data
	if (temp <= 0) {
