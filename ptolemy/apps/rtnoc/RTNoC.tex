...
function of $l$: $s_{g,e}=f(l_g)$. We can set $s_{f,e}=value(l_e)$, this means
...
$p_{f,e}$ is the {\em propagation} delay of a {\em flit} or a {\em packet},
since real-time packets are not preemptible, of flow $f$ on link $e \in E$. We
assume in our network on chip, $p_{f,e}=1 \forall e \in E$.
...
%FIXME
%Configuration $\mathcal{C}(F)=(P, \{ y_{f,e}:f \in F, e \in E \} )$.
