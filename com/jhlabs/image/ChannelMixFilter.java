...
public class ChannelMixFilter extends PointFilter {
    ...
    private int blueGreen = 127, redBlue = 127, greenRed = 127;
    private int intoR = 255, intoG = 255, intoB = 255;
    ...
    public int filterRGB(int x, int y, int rgb) {
        ...
        int b = rgb & 0xff;
        int nr = PixelUtils
                .clamp((intoR * (blueGreen * g + (255 - blueGreen) * b) / 255
                        + (255 - intoR) * r) / 255);
        int ng = PixelUtils
                .clamp((intoG * (redBlue * b + (255 - redBlue) * r) / 255
                        + (255 - intoG) * g) / 255);
        int nb = PixelUtils
                .clamp((intoB * (greenRed * r + (255 - greenRed) * g) / 255
                        + (255 - intoB) * b) / 255);
