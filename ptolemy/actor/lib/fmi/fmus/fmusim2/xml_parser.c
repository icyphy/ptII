...
const char* getModelIdentifier(ModelDescription* md) {
    const char* modelId = NULL;
#if FMI_VERSION >= 2
    modelId = getString(md, att_modelName);
#else
    modelId = getString(md, att_modelIdentifier);
#endif
...
#if FMI_VERSION >= 2
                 if (child->type == elm_CoSimulation) {
                     cs = (CoSimulation*)child;
                     child = checkPop(ANY_TYPE);
                     if (!child) return;
                 }
...
#if FMI_VERSION >= 2
            printElement(indent, cs->attributes);
#else
...
void freeElement(void* element){
    ...
    switch (getAstNodeType(e->type)) {
        ...
        case astModelDescription: {
            ...
            // FIXME: not freeing cosimulation struct.
