/* ---------------------------------------------------------------------------*
 * FMU for an ideal heater with flow resistance.
 *
...
#define MODEL_GUID "{41291936-a3a0-4f94-b326-9be08315fe8d}"
...
FMI_Export fmiComponent fmiInstantiate(fmiString instanceName,
    ...
    printf("heaterFlowResistance.c: exit fmiInstantiate.\n");
    return component;
}
...
FMI_Export fmiStatus fmiGetContinuousStates(fmiComponent c, fmiReal x[],
        size_t nx) {
    printf("heaterFlowResistance.c: fmiGetContinuousStates() called, even though the FMU has no states.\n");
    return fmiOK;
}
...
FMI_Export fmiStatus fmiGetDerivatives(fmiComponent c, fmiReal derivatives[],
        size_t nx) {
    // Model Exchange
    printf("heaterFlowResistance.c: fmiGetDerivatives() is called even though the FMU has no states.\n");
    return fmiOK;
}
...
FMI_Export fmiStatus fmiSetContinuousStates(fmiComponent c, const fmiReal x[],
        size_t nx) {
    // Model Exchange
    printf("heaterFlowResistance.c: fmiSetContinuousStates() is called even though the FMU has no states.\n");
    return fmiOK;
}
