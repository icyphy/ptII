                        ...
                        int end_index, char *table_size_str)
{
    ...
    if (jflag) {
       ...
       fprintf(code_file, "final static short %s[] = read_short_table(\"%s\", %s);\n",
...
int j;
    ...
    if (jflag) { /*rwj*/
      ...
      fprintf(code_file, "final static short yylhs[] = read_short_table(\"yylhs\", NRULES - 2);\n");
...
int i, j;
    ...
    if (jflag) {
      ...
      fprintf(code_file, "final static short yydefred[] = read_short_table(\"yydefred\", NSTATES);\n");
    ...
    for (i = 0; i < nstates; i++) {
	     ...
	     fprintf(table_file, num_format_str, (defred[i] ? defred[i] - 2 : 0));
    }
...
FILE *f;
  if (jflag)/*rwj*/
    ...
    fprintf(f,"  obj=val;\n");
    fprintf(f,"}\n");
    fprintf(f,"public Object clone() throws CloneNotSupportedException\n");
    fprintf(f,"{\n");
