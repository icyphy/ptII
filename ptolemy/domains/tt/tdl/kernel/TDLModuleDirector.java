...
import ptolemy.domains.tt.tdl.kernel.TDLModeSchedule;
import ptolemy.domains.tt.tdl.kernel.TDLModeScheduler;
import ptolemy.domains.tt.tdl.kernel.TDLModeSchedulerException;
import ptolemy.domains.tt.tdl.kernel.TDLModule;
import ptolemy.domains.tt.tdl.kernel.TDLReceiver;
 ...
 * 
 * Schedule is generated statically in the initialize method
 * after setting output ports and actuators and after each slot
 * in the schedule, the control is handed to the outside director to 
 * enable executing other actors outside the TDL module
 *
 * @author Patricia Derler
 */
public class TDLModuleDirector extends ModalDirector {

    public TDLModuleDirector(CompositeEntity container, String name) throws IllegalActionException, NameDuplicationException {
        super(container, name);
    }
    
    /**
     * pick the next action in the schedule and execute it
     * leave the fire method 
     * - if mode switch has to be made
     * - before input ports are read and updated
     * - every part of the schedule for the current time was executed
     *   (= a slot in the schedule was executed)  
     */
...
//        if (_debugging)
        ...
        int i = -1; // used to avoid returning control to the executive director before every input port update
        for (; _currentSchedule.currentPositionInSlot < _schedule.size(); _currentSchedule.currentPositionInSlot++) {
            ...
            if (!_hasGuard((NamedObj) obj) || _guardIsTrue((NamedObj) obj)) {
	            if (obj instanceof Actor) {
	                ...
	                if (actor.prefire()) { // TODO prefiere should never return false ?
	                    actor.iterate(1);
	                    actor.postfire();
	                }               
	                ...
	                if (_isActuator(port)) {
	                	...
	                	// skip updating of output ports if the module is executed the first time
	                	// or after a mode switch 
	                	// only if task is not a fast task
	                    if (_currentSchedule.firstSlot && !TDLModuleDirector.isFast(port.getContainer())) 
	                        continue;      
	                    _updateOutputPort(port);
	                } else if (port.isInput()) {
	                    Director executiveDirector = ((Actor)getContainer().getContainer()).getDirector();
	                    if (i > 0) { 
	            	...
	            	// skip mode switches if mode is executed the first time or immediately after
	            	// a mode switch
        ...
        if (continueSchedule) {
            ...
            Time t = new Time(this, (long) (getModelTime().getLongValue() + nextTimeStamp));
            fireAt((TDLModule)getContainer(), t);
            _currentSchedule.nextFireTime = t.getLongValue();
            _currentSchedule.currentPositionInSlot = 0;
            ((TDLModeSchedule)_modeSchedules.get(getController().currentState())).currentScheduleTime = 
                (((TDLModeSchedule)_modeSchedules.get(getController().currentState())).currentScheduleTime + nextTimeStamp) %
                ((TDLModeSchedule)_modeSchedules.get(getController().currentState())).modePeriod; 
        }
    }
    ...
    public void initialize() throws IllegalActionException {
        ...
        _currentSchedule = (TDLModeSchedule) _modeSchedules.get(getController().currentState());
            ...
            try {
                if (i < port.getWidthInside()) {
                    if (port.hasToken(i)) {
                        Token t = port.get(i);
                        if (_debugging) {
                            _debug(getName(), "transferring input from " + port.getName());
                        }
                        port.sendInside(i, t);
                        wasTransferred = true;
                    }
    ...
    /**
     * iterate through states and tasks and add everything to the scheduler 
     ...
     */
    private void _buildSchedule(TDLModeScheduler scheduler, State state) throws IllegalActionException, TDLModeSchedulerException {        
    ...
    private boolean _chooseTransition(Transition transition) throws IllegalActionException {
        ...
        // update sensors
        Refinement ref = (Refinement) getController().currentState().getRefinement()[0];
        ...
        if (transition.isEnabled()) {
            ...
            fireAt((TDLModule)getContainer(), getModelTime());
            _currentSchedule.firstSlot = true;
            ((TDLModeSchedule) _modeSchedules.get(transition.destinationState())).firstSlot = true;
            ((TDLModeSchedule) _modeSchedules.get(transition.destinationState())).nextFireTime = getModelTime().getLongValue();
            _currentSchedule.currentPositionInSlot = 0;
    ...
    private boolean _executeNow() throws IllegalActionException {
        ...
        _schedule = (ArrayList)_currentSchedule.modeSchedule.get(_currentSchedule.currentScheduleTime);
        if (_schedule == null || _schedule.size() == 0) {
            return false; // nothing to do here
        }
        
        if (_currentSchedule.lastFiredAt == currentTime && _schedule.size() == _currentSchedule.currentPositionInSlot)
    ...
    private long _getNextTimeStamp() throws IllegalActionException {
        SortedSet keys = new TreeSet(((TDLModeSchedule)_modeSchedules.get(getController().currentState())).modeSchedule.keySet());
        Iterator it = keys.iterator();
        long currentScheduleTime = ((TDLModeSchedule)_modeSchedules.get(getController().currentState())).currentScheduleTime;
        long currentModePeriod = ((TDLModeSchedule)_modeSchedules.get(getController().currentState())).modePeriod;
        while (it.hasNext()) {
            long d = (Long)it.next();
	...
	private boolean _guardIsTrue(NamedObj obj) throws IllegalActionException {
        ...
        Token tok = parseTreeEvaluator.evaluateParseTree(_guardParseTree, fsmActor.getPortScope());
	...
	private void _initializePort(IOPort port) throws IllegalActionException {
		Parameter initialValueParameter = (Parameter) ((NamedObj) port).getAttribute("initialValue");
        Token token;
    ...
    /**
     * initialize output ports by reading initialvalue and initializing the receivers
     * @throws IllegalActionException
     */
    ...
    /**
     * read input values and update inputMap 
     * the updated inputMap is required when guards are evaluated 
     * @throws IllegalActionException
     */
        ...
        while(it.hasNext()) {
            ...
            for (int j = 0; j < list.size(); j++) {
                if (list.get(j) instanceof TypedCompositeActor) {
                    ...
                    System.out.print(transition.guardExpression.getExpression());
                } else
    ...
    private void _printStatus() throws IllegalActionException {
        ...
        formatter.format("%1$5f state %2$10s schedTime %3$2d of %4$2d schedPos %5$2d of %6$2d", 
                new Object[]{
                new Double(d), 
    ...
    private void _transferTaskInputs(Transition transition) throws IllegalActionException {
        Refinement oldRefinement = (Refinement) getController().currentState().getRefinement()[0];
        Refinement newRefinement = (Refinement) transition.destinationState().getRefinement()[0];
