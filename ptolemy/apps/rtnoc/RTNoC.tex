...
\title{On-Time Network On-Chip: Analysis and Architecture}
...
Their combination with network on chip would be very promissing similarly to
J-machine. However, the more cores, the more communication unpredictability
since congestions is a big issues in packet-switch networks today. One question
is how to use multi-core and many-core in real-time embedded systems? How does
a critical application know for sure that one packet sent from a node will
reach the destination node within a certain amount of time? Such type of
applications always require bounded delay in communications.In such a case,
faster does not always imply better.In multi-core, communication is often more
expensive than computation. The communication overhead often greatly reduces
the performance of a parallel application. An explicit communication mechanism
between cores running critical tasks is also a must. This mechanism needs to be
simple yet efficient and flexible enough for most of applications.
...
implemented on a cycle-accurate simulation platform is used to prove the
worst-case delay estimation of a deadline-based packet scheduling algorithm.
...
\section{Problem Settings and Definitions}
...
l_f}s_{g,e}y_{g,e} +max_{l_i>l_f}(s_{i,e}y_{i,e})\Biggr)\notag \\ \leq
...
\sum_{e \in E} \Biggl(\sum_{g \in F:l_g <
l_f}value(l_g)y_{g,e} +max_{l_i>l_f}(value(l_i)y_{i,e})\Biggr)\notag \\ \leq
...
\begin{eqnarray}\label{equ:slack1}
	...
	\sum_{e \in E} \Biggl(\sum_{g \in F:l_g < l_f}value(l_g)y_{g,e}
...
\sum_{e \in E} \Biggl(\sum_{g \in F:l_g <
l_f}s_{g,e}y_{g,e}+s_{f',e}y_{f',e}+max_{l_i>l_f}(s_{i,e}y_{i,e})\Biggr)\notag
...
\sum_{\forall e \in E}s_{f',e}y_{f',e} \leq d_f-s_{f,last\_edge} + 1 - \notag \\ 
\sum_{e \in E} \Biggl(\sum_{g \in F:l_g <
l_f}s_{g,e}y_{g,e}+s_{f',e}y_{f',e}+max_{l_i>l_f}(s_{i,e}y_{i,e})\Biggr) \notag
\\ \forall f \in F
\end{eqnarray}
...
\sum_{\forall e \in E}s_{f',e}y_{f',e} \leq slack_f, \forall f \in F
\end{equation}

or

\begin{equation}\label{equ:e2eDelayModificatio4}
\sum_{\forall e \in E}value(l_{f'})y_{f',e} \leq slack_f, \forall f \in F
\end{equation}
...
\section{Architecture and Specifications}
...
\begin{figure}[htp]
\centering
\includegraphics[width=9cm]{pics/PE23.pdf}
...
\begin{figure}[htp]
\centering
\includegraphics[width=9cm]{pics/PE3.pdf}
\caption[Three flows example.]
{Real delay and estimated delay of real-time flow from node
...
\begin{figure}[htp]
\centering
\includegraphics[width=9cm]{pics/PE19.pdf}
\caption[Three flows example.]
{Real delay and estimated delay of real-time flow from node
...
a real-time multi-core system. Furthermore, if we use PRET, we can make sure
the minimum interval between two successive packets easily with deadline
instructions.
