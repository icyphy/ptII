...
// Stop extra messages from jslint. Note that there should be no // space between the / and the * and global.
...
exports.setup = function() {
    ...
    // FIXME: This should be a parameter? RosPublisher.input('server', {
    //'value': '192.168.0.103' 'value': '192.168.1.201' });
    ...
    // FIXME: This should be a parameter? RosSubscriber.input('server', {
    //'value': '192.168.0.103' 'value': '192.168.1.201' });
    RosSubscriber.input('port', {
        'value': '9090'
    });
    RosSubscriber.setParameter('topic', '/cmd_vel');
    //Workaround so this composite can set an input handler for ros data this.connect(RosSubscriber, "received", this, "velocities");
};
...
exports.initialize = function() {
    ...
    this.addInputHandler('velocities', function (){
        ...
        var data = {
            "Angular Velocity (rad/s)": vels.msg.angular.z, "Linear Velocity (m/s)": vels.msg.linear.x }
        ;
        this.send('data', data);
    });
    // At initialize, send the schema;
    this.send('schema', schema);
    // Also send null data. this.send('data', null);
};

exports.wrapup =function(){
    //Reset the data display this.send('data', null);
    RosPublisher.wrapup();
...
var schema = {
    "type": "object", "properties": {
        "action": {
            "type": "string", "title": "Robot Action", "description": "Command for the robot to perform.", "choices": ["forward", "spin", "patrol"] }
    }
};
