...
public class SortMembersObjectAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (_lastSelection != null
				&& _lastSelection instanceof ITreeSelection) {
			ITreeSelection treeSelection = (ITreeSelection) _lastSelection;
			Iterator iterator = treeSelection.iterator();
			while (iterator.hasNext()) {
				ICompilationUnit compilationUnit =
					(ICompilationUnit) iterator.next();
				try {
					IEditorPart editor =
						EditorUtility.openInEditor(compilationUnit, false);
					if (editor != null) {
						SortMembersUtility.sortICompilationUnit(compilationUnit,
								editor);
					}
					if (!editor.isDirty()) {
