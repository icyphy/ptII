...
$\forall f \in F  that l_{f'} < l_f$ and $y_{f',e}u{f,e}=1 \mbox{ for some } e
\in E$, from (\ref{equ:e2eDelayCTNonPipeline1}) we have:

\begin{equation}\label{equ:e2eDelayModification}
\sum_{e \in E} (\sum_{\forall g \in F:l_g <
l_{f'}}y_{g,e}s_{g,e}+max(s_{i \in F}.y_{i,e}))y_{f',e} +y_{f',e}s_{f',e} \leq
...
y_{f',e}s_{f',e} \leq P(f)(4)-P(f)(3) + 1 - \sum_{e \in E} (\sum_{\forall g \in
F:l_g < l_{f'}}y_{g,e}s_{g,e}+max(s_{i \in F}.y_{i,e}))y_{f',e}, \forall f \in F
\end{equation}

or

\begin{equation}\label{equ:e2eDelayModificatio3}
y_{f',e}s_{f',e} \leq slack_f, \forall f \in F
\end{equation}
