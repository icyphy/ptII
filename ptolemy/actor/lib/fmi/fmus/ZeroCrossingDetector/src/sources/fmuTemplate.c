...
fmi2Status fmi2GetHybridReal (fmi2Component c, const fmi2ValueReference vr[], size_t nvr, fmi2Real value[], fmi2Integer hybridValue[]) {
    ...
    for (i = 0; i < nvr; i++) {
        ...
        if (hybridValue[i] == absent_) {
            FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2GetHybridReal: #r%u# = ABSENT at time (%u, %u)", vr[i], comp->time, comp->microstep)
        }
        else {
            FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2GetHybridReal: #r%u# = %.16g at time (%u, %u)", vr[i], value[i], comp->time, comp->microstep)
        }
    }
#endif
    return fmi2OK;
...
fmi2Status fmi2SetHybridReal (fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2Real value[], const fmi2Integer hybridValue[]) {
    ...
    for (i = 0; i < nvr; i++) {
        if (vrOutOfRange(comp, "fmi2SetHybridReal", vr[i], NUMBER_OF_REALS))
            return fmi2Error;
        if (hybridValue[i] == absent_) {
            FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetHybridReal: #r%u# = ABSENT at time (%u, %u)", vr[i], comp->time, comp->microstep)
        }
        else {
            FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetHybridReal: #r%d# = %.16g at time (%u, %u)", vr[i], value[i], comp->time, comp->microstep)
        }
        comp->r[vr[i]] = value[i];
...
fmi2Status fmi2HybridDoStep(fmi2Component c, fmi2Integer currentCommunicationPoint,
    ...
    if (ei < 0) {
        FILTERED_LOG(comp, fmi2OK, LOG_EVENT,
            "fmi2HybridDoStep: state event at (%u, %u), crosses zero -%c-",
            comp->time,
            comp->microstep,
            ei < 0 ? '\\' : '/')
        stateEvent++;
    ...
    } else if (communicationStepSize == 0) {
...
#if NUMBER_OF_EVENT_INDICATORS>0
    for (i = 0; i < ni; i++) {
        eventIndicators[i] = getEventIndicator(comp); // to be implemented by the includer of this file
