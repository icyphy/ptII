...
fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint,
    ...
    //         ((comp->time - comp->eventInfo.nextEventTime) > -0.0000000001)) {
    //     FILTERED_LOG(comp, fmi2Discard, LOG_EVENT,
    //         "fmi2DoStep: time event missed at (%g, %u),  it was at %g",
    //         comp->time,
    //         comp->microstep,
    ...
    if (comp->eventInfo.nextEventTimeDefined &&
            ((comp->time - comp->eventInfo.nextEventTime) > -0.000000000000001 &&
            ((comp->time - comp->eventInfo.nextEventTime) <  0.000000000000001))) {
        FILTERED_LOG(comp, fmi2OK, LOG_EVENT,
            "fmi2DoStep: time event detected at (%g, %u)",
            comp->time,
            comp->microstep)
        timeEvent = 1;
    }
    ...
    if (comp->eventInfo.terminateSimulation) {
        FILTERED_LOG(comp, fmi2Discard, LOG_ALL,
            "fmi2DoStep: model requested termination at t=(%g, %u)",
            comp->time,
            comp->microstep)
    ...
    if (!(communicationStepSize > -0.000000000000001 && communicationStepSize < 0.000000000000001)) {
        comp->microstep = 0;
    } else {
        comp->microstep++;
    }
...
fmi2Status fmi2SetResolution (fmi2Component c, fmi2IntegerTime value) {
    return fmi2OK;
}
...
fmi2Status fmi2GetMaxStepSize (fmi2Component c, fmi2Real *value) {
    ...
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2GetMaxStepSize: "
        "currentCommunicationPoint = (%g, %u), "
        "communicationStepSize = %g, ",
