...
void timeAdvanceGrant(const RTI::FedTime& theTime)
    ...
    if (true){ 
        cout << " >> TAG(" << _LocalTime << ") RECU == LocalTime = " << _LocalTime << " << " << endl; 
...
int main() {
    ...
    string federateName   = "producerFed";
    ...
    try {
        valID = rtiAmb.getAttributeHandle("VAL3", myObjectID);
    } catch ( RTI::Exception &e ) {
        cerr << "RTI exception: " << e._name << " ["
        << (e._reason ? e._reason : "undefined") << "]." << endl;
    } catch ( ... ) {
   ...
   while ( !myFedAmb.Get_IsTimeConst()) {
    ...
    if(SYNCHRO_INITIALISATION){
        cout << " "                           << endl;
        cout << ">> Simulating the federate: p3Fed" << endl;  
    ...
    while (cpt < 5) {
	  ...
	  rtiAmb.updateAttributeValues(objInstID_val, *ahvps, myFedAmb.Get_LocalTime()+myFedAmb.Get_Lookahead() ,"VAL");
        ...
        cout << "Federate p3Fed has updated the value VAL3: " << cpt << endl;
