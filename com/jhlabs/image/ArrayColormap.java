    ...
    * @see #setColor
    ...
    public void setColorInterpolated(int index, int firstIndex, int lastIndex,
        ...
        for (int i = firstIndex; i <= index; i++) {
            map[i] = ImageMath.mixColors(
                    (float) (i - firstIndex) / (index - firstIndex), firstColor,
                    color);
        }
        for (int i = index; i < lastIndex; i++) {
            map[i] = ImageMath.mixColors(
                    (float) (i - index) / (lastIndex - index), color,
                    lastColor);
        }
    }
    ...
    public void setColorRange(int firstIndex, int lastIndex, int color1,
        ...
        for (int i = firstIndex; i <= lastIndex; i++) {
            map[i] = ImageMath.mixColors(
                    (float) (i - firstIndex) / (lastIndex - firstIndex), color1,
                    color2);
        }
    }
