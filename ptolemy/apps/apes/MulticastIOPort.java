...
This port communicates without wired connections. 
The port references the destinations and sources by names
as specified by the <i>destinationActors</i> or <i>sourceActors</i> parameter.
<p>
A port is outside multicast if at least one destination actor is given and
a NamedDestinationActor with the given name is contained by the container
of the port's container (transparent hierarchy is not supported).
A port is inside multicast if at least one destination actor is given and
a NamedDestinationActor with the given name is contained by the container
of this port.
If no destination actor is given, then the behavior of the port reverts
to that of the base class. Specifically, it will only communicate if it is wired.
If at least one destination is given and the actor exists, then all the wired
connections to this port are ignored.
<p>
The width of this port on either side that is using multicast
communication is fixed at one. Otherwise, it depends on the
number of links to the port.
<p>
When this port is used for multicast communications, nothing is
...
list. There are no deeply connected ports.
...
public class MulticastIOPort extends TypedIOPort {
  ...
  /** A comma-separated list of actors that can send tokens to this port through
   * multicast ports.
   * This is a string that defaults to the * string, indicating that all
   * actors are accepted.
   */
...
private long _destinationActorsVersion;
  ...
  public void attributeChanged(Attribute attribute)
      ...
      if ((attribute == destinationActors)
              || (attribute == sourceActors)) {
  ...
  public void broadcast(Token token) throws IllegalActionException {
      ...
      Collection<IOPort> destinationActorPorts = getAllDestinationActors();
  ...
  private Collection<IOPort> getDestinationActors() {
  ...
  private Collection<IOPort> getAllDestinationActors() {
      if (workspace().getVersion() != _destinationActorsVersion) {
          // TODO catch exceptions if getContainer() fails
          CompositeActor compositeActor = (CompositeActor) getContainer().getContainer();
          List entities = compositeActor.entityList();
  ...
  public void send(int channelIndex, Token token)
      ...
      Collection<IOPort> destinationActorPorts = getDestinationActors();
