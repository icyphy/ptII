...
/***numberBlock($channel)***/
    ...
    if ($actorSymbol(count) < $size(correctValues) && fabs($ref(input#$channel) - $ref(correctValues, $actorSymbol(count))) > $ref(tolerance)) {
        // FIXME: what about types other than double?
        printf("Test fails in iteration %d.\n Value was: %f. Should have been: %f\n",
            $actorSymbol(count), (double)$ref(input#$channel), (double)$ref(correctValues, $actorSymbol(count)));
        exit(-1);    
    }
    $actorSymbol(count) ++;
/**/
...
/***stringBlock($channel)***/
	if (strcmp($ref(correctValues, $actorSymbol(count)), $ref(input#$channel))) {
