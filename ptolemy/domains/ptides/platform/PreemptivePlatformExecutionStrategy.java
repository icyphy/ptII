...
public class PreemptivePlatformExecutionStrategy extends
	...
	public boolean actorCanBeFired(Actor actorToFire, Time nextRealTimeEvent) {
		return true; // assuming that this event was selected before with the isSafeToProcess method
	}
	...
	public boolean dealWithNewlyReceivedEvents(Actor actorToFire, Hashtable eventQueues) {
		// if there exists an event that should be fired before the currently executing Actor
		// TODO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		return false;
	}
	...
	public boolean missedExecution(Actor actorToFire, Time fireTime) {
		return fireTime.add(getExecutionTimeWithPreemption((NamedObj)actorToFire)).compareTo(_physicalTime) <  0;
	}

	private static double getExecutionTimeWithPreemption(NamedObj actor) {
		try {
            Parameter parameter = (Parameter) actor.getAttribute("preemptedExecutionTime");
	...
	public static void setExecutionTimeWithPreemption(IOPort out, double preemptedExecutionTime) throws IllegalActionException {
    	Parameter parameter = (Parameter) ((NamedObj)out).getAttribute("preemptedExecutionTime");
		if (parameter == null)
			try {
			parameter = new Parameter((NamedObj)out, "preemptedExecutionTime", new DoubleToken(preemptedExecutionTime));
	...
	private class WCETComparator implements Comparator {
		...
		public int compare(Object arg0, Object arg1) {
			...
			if (time1.add(wcet1).compareTo(wcet2) > 0)				return -1;
			if (time2.add(wcet2).compareTo(wcet1) > 0)				return 1;
			if (wcet1 == 0 && wcet2 == 0) {
