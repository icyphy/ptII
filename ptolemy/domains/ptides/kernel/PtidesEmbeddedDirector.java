...
public class PtidesEmbeddedDirector extends DEDirector {
     ...
     */
	public PtidesEmbeddedDirector(CompositeEntity container, String name) throws IllegalActionException, NameDuplicationException {
    ...
    public void attributeChanged(Attribute attribute)
        ...
        if (attribute == executionStrategy) {
			 String strategy = ((StringToken) executionStrategy.getToken()).stringValue();
			 if (strategy.equals(PlatformExecutionStrategy.BASIC_NON_PREEMPTIVE)) {
				 _executionStrategy = new NonPreemptivePlatformExecutionStrategy(_physicalTime, this);
			 }
		 } else {
            super.attributeChanged(attribute);
        }
	...
	public final void displaySchedule(Actor actor, double time,
		...
		if (dir != null)
			dir._displaySchedule((Actor) getContainer(), actor, time, scheduleEvent);
	}
	...
	/** 
	 * Return the current model time. This director does not have a valid model time
	 * except during the time an actor is being fired. Then, the model time is the
	 * time of the event which caused the firing of that actor.
	 */
	...
	/**
	 * Until _stopRequested, the director tries to fire actors or waits. 
	 * First, a set of events is calculated that can be fired at current time. 
	 * Out of this set of events, one event is chosen that is fired. The actor
	 * for this event is added to the list of actors in execution and its deadline
	 * is set to event.timestamp() + wcet. After the expiration of the deadline, 
	 * the actor is removed from the list and the fire() of the actor is executed. 
	 * If no actor is found that can execute now, this actor waits. It continues 
	 * execution when the phyiscal time is increased or when tokens are received on 
	 * the input ports.
	 * If an actor is preempted, the preempting actor is added to the list of 
	 * actors in execution and the deadlines for all other actors in execution are
	 * updated to oldDeadline + wcet of the preempting actor. 
	 */
	public void fire() throws IllegalActionException {
        ...
        while (true) {
        	if (_actorsInExecution.size() > 0) {
        		Actor actorToFire = (Actor) _actorsInExecution.get(_actorsInExecution.size() - 1);
        		...
        		if (d == _physicalTime.getDoubleValue()) {
        			...
        			displaySchedule(actorToFire, _physicalTime.getDoubleValue(), ScheduleListener.STOP);
        			actorToFire.fire();
            ...
            for (int i = 0; i < eventsToFire.size(); i++)
            	_addSynchronizationPoint(((DEEvent)eventsToFire.get(i)).timeStamp());
            event = _executionStrategy.getNextEventToFire(_actorsInExecution, eventsToFire);
            ...
            if (event != null) {
            	...
            	if (!actorToFire.prefire()) {
	            	...
	            	displaySchedule(actorToFire, _physicalTime.getDoubleValue(), ScheduleListener.START);
	            	System.out.println(this.getContainer().getName() + "-fired " + actorToFire.getName());
	        		_addSynchronizationPoint(_physicalTime.add(wcet));
	        		setDeadline(actorToFire, _physicalTime.add(wcet).getDoubleValue());
                ...
                _setPhysicalTime(_getExecutiveDirector().waitForFuturePhysicalTime());
                if (_stopRequested)
                	return;
                if (_transferAllInputs()) {
                	_enqueueRemainingEventsAgain(eventsToFire);
	 ...
	 * the actors inside if they are safe to process according to PTIDES.
	 * Tokens are safe to process if 
	 * eventTimestamp - minDelay 
	 *                + clockSyncError 
	 *                + networkDelay <= physicalTime
	 * @param time Timestamp of the event.
	 * @param object Port or Actor
	 ...
	 * timestamp - minimumDelay <= physicalTime.
	 * The minimum delay must be specified als a parameter at the port.
	 * @param time
	 * @param object
	 * @return
	 */
    ...
    public void preinitialize() throws IllegalActionException {
        super.preinitialize(); // bad style, but has to be done because otherwise the eventqueue that is not used is not initialized and this causes problems in wrapup
        ...
        _physicalTime = ((Actor)getContainer()).getExecutiveDirector().getModelTime();
    ...
    /**
     * the top level director (PtidesDirector) sets the clock synchronization error.
     * @param syncError
     */
     ...
     * @throws IllegalActionException
     */
    public void setDeadline(Actor actor, double deadline) throws IllegalActionException {
		if (_deadlinesOfActorsInExecution.get(actor) != null)
...
//	    	((DEEventQueue)_eventQueues.get(actor)).put(newEvent);
	...
	/** Put an event into the event queue for the given actor to
	 *  fire at the specified timestamp.
	 *  The depth for the queued event is the minimum of the depths of
	 *  all the ports of the destination actor.
	 *  If there is no event queue or the given actor is disabled, then
...
//			((DEEventQueue)_eventQueues.get(actor)).put(newEvent);	
	...
	/**
	 * Transfer input ports if they are safe to process. If they token is not safe
	 * to process, inform the executive director to be refired at the time it is
	 * safe to process the token.  
	 */
            ...
            try {
                while (port.hasToken(i)) {
                    ...
                    if (time.compareTo(_physicalTime) < 0)
                    	throw new IllegalActionException("Network interface constraints violated at " + this.getContainer().getName() + ", tried to transfer event with timestamp " + time + " at physical time " + _physicalTime);
                    if (!((_usePtidesExecutionSemantics && isSafeToProcessOnNetwork(time, port)) ||
                    		(_usePtidesExecutionSemantics && time.compareTo(_physicalTime) > 0))) {
                  
                    	System.out.println("cannot ti yet " + time);
                    	_addSynchronizationPoint(time.subtract(PtidesGraphUtilities.getMinDelayTime(port))
                				.add(_clockSyncError)
                				.add(_networkDelay)); // at this time, the new input should be read
                    	receiver.put(t, time);
                    ...
                    for (int k = 0; k < farReceivers.length; k++) {
                        for (int l = 0; l < farReceivers[k].length; l++) {
                            ...
                            displaySchedule((Actor) port.getContainer(), _physicalTime.getDoubleValue(), ScheduleListener.TRANSFERINPUT);
            	...
            	if (receivers[i] != null) {
                    for (int j = 0; j < receivers[i].length; j++) {
                        if (receivers[i][j] != null && 
                        		((PtidesDEReceiver)receivers[i][j]) .getNextTime() != null && 
                        		((PtidesDEReceiver)receivers[i][j]) .getNextTime().compareTo(_physicalTime) < 0) {
                        	throw new IllegalActionException("Network constraints violated, token with timestamp " +  ((PtidesDEReceiver)receivers[i][j]) .getNextTime() 
                    				+ " cannot be transferred to the output at physical time " + _physicalTime);
                        }
                    }
            	}
                if (port.hasTokenInside(i)) {
                    ...
                    for (int k = 0; k < outReceivers.length; k++) {
                        for (int l = 0; l < outReceivers[k].length; l++) {
                            ...
                            displaySchedule((Actor)port.getContainer(), _physicalTime.getDoubleValue(), ScheduleListener.TRANSFEROUTPUT);
                            System.out.println("	out " + token + " at rt: " + _physicalTime + "/mt: " + getModelTime());
                        }
                    }
                    result = true;
                }
    ...
    private void _addSynchronizationPoint(Time time) throws IllegalActionException {
    	((Actor)getContainer()).getExecutiveDirector().fireAt((Actor) this.getContainer(), time);
	}
	...
	/**
	 * Get the set of events that are safe to fire. Those events contain pure events and
	 * triggered events.
	 * @return
	 * @throws IllegalActionException
	 */
	private List _getNextEventsToFire() throws IllegalActionException {
		...
		for (Enumeration actors = _eventQueues.keys(); actors.hasMoreElements();) {
			...
			if (!queue.isEmpty()) {
				...
				boolean isSafe = isSafeToProcessOnPlatform(event.timeStamp(), (NamedObj)actor);
    			if (isSafe) {
    				events.add(event);
    				queue.take();
    			}
				...
				for (Iterator it = actor.inputPortList().iterator(); it.hasNext(); ) {
					...
					for (int i = 0; i < receivers.length; i++) {
						...
						for (int j = 0; j < recv.length; j++) {
							...
							if (time != null && isSafeToProcessOnPlatform(time, port)) {
								_removePureEvent(events, actor, time);
								events.add(new DEEvent(port, time, 0, _getDepthOfIOPort(port)));
							}
						}
					}
				}
	...
	/**
	 * if there is a pure event and a triggered event for the same actor with the same timestamp, 
	 * then the pure event can be deleted
	 * @param events
	 * @param actor
	 * @param time
	 */
	private void _removePureEvent(List events, Actor actor, Time time) {
		...
		for (int i = 0; i < events.size(); i++) {
			...
			if (event.ioPort() == null && event.actor().equals(actor) && event.timeStamp().equals(time)) 
				toRemove.add(event);
		}
	 ...
	 * 	private List _getNextEventsToFire() throws IllegalActionException {
		...
		for (Enumeration actors = _eventQueues.keys(); actors.hasMoreElements();) {
			...
			DEEventQueue queue = (DEEventQueue) _eventQueues.get(actor);
			ArrayList taken = new ArrayList();
			//this gets all events that are safe to fire - required if only one eventqueue at the actors is used
			if (!queue.isEmpty()) {
				DEEvent event = queue.get();
				while (!queue.isEmpty()) {
	    			event = queue.take();
...
//				}
//			}
			for (Iterator it = taken.iterator(); it.hasNext(); ) {
				DEEvent event = (DEEvent) it.next();
				_enqueueEventAgain(event);
			}
		}
    ...
    private void _initialize() {
		try {
			...
			executionStrategy.addChoice(PlatformExecutionStrategy.BASIC_NON_PREEMPTIVE);
			executionStrategy.setExpression(PlatformExecutionStrategy.BASIC_NON_PREEMPTIVE);
