...
public class TDLModeScheduler extends LetModeScheduler {
     ...
     */
    public void addActuator(IOPort port) throws IllegalActionException, TDLModeSchedulerException {
    ...
    private void analyzeSlotSelection(Actor actor, String slots, int frequency) throws TDLModeSchedulerException {    	
    	ArrayList invocations = getInvocations(slots, frequency);
    	...
    	if (frequency % (invocations.size() / 2) != 0) {
	    	...
	    	for (int i = 0; i < invocations.size(); i += 2) {
	    		newlet = Math.abs((Integer) invocations.get(i + 1) - (Integer) invocations.get(i));
	    		if (let == 0)
	    			let = newlet;
	    		else if (newlet != let)
	    			periodic = false;
	    	...
	    	if (invocations.size() > 2) {
		    	for (int i = 0; i < invocations.size(); i += 2) {
		    		if (invocations.size() > i + 2) {
			    		newInv = (Integer) invocations.get(i + 2) - (Integer) invocations.get(i);
		    		} else 
    	...
    	if (periodic) {
    		offset = (_modePeriod / frequency) * ((Integer) invocations.get(0) - 1);
    		let = _modePeriod / let;
    		inv = _modePeriod / inv;
    		...
    		for (int i = 0; i < invocations.size(); i += 2) {
    			let = _modePeriod / frequency * ((Integer) invocations.get(i + 1) - (Integer) invocations.get(i));
    			offset = _modePeriod / frequency * ((Integer) invocations.get(i) - 1);
    			super.addTask(new LetTask(actor, let, _modePeriod, offset));
    		}
    	}
    }
    
    private ArrayList getInvocations(String slots, int frequency) throws TDLModeSchedulerException {
    		...
    		switch (slotSelection.charAt(i)) {
    			...
    			case '9':
    				...
    				if (slotSelection.length() > i + 1 && 
    						slotSelection.charAt(i + 1) > 47 && slotSelection.charAt(i + 1) < 58)
    					number += slotSelection.charAt(++i);
    				...
    				if (firstSlot == 0) {
    					...
    					if (hadStar) { // everthing between last invocation and 'firstSlot' is scheduled the same way as the last slotGroup
        					int lastStart = (Integer) invocations.get(invocations.size() - 2);
        					int lastEnd = (Integer) invocations.get(invocations.size() - 1);
    			...
    			case '*':
    				hadStar = true;
    				if (slotSelection.length() >= i && slotSelection.charAt(i + 1) == '|')
    					break;
    			case '|':
    			...
    			case '\'': // start and end, to avoid errors in parameter "expression could not be parsed"
    				break;
    			default:
    				throw new TDLModeSchedulerException("'" + slots + "' cannot be parsed");
    		}
    	}
     ...
     * @throws TDLModeSchedulerException 
     */
    public void addTask(Actor actor) throws IllegalActionException, TDLModeSchedulerException {
    	int frequency = TDLModeScheduler.getFrequency((NamedObj) actor);
    	Parameter parameter = (Parameter) ((NamedObj)actor).getAttribute("slots");
    	if (parameter != null) {
	    	String token = parameter.getExpression();
	    	analyzeSlotSelection(actor, token, frequency);
    	}
//    	long let = _modePeriod / frequency;
//    	long invocationPeriod = _modePeriod / TDLModeScheduler.getFrequency((NamedObj) actor);
//        super.addTask(new LetTask(actor, let, invocationPeriod, 0));
    }
     ...
     * @throws IllegalActionException 
     */
    private void _createTDLSchedule() throws TDLModeSchedulerException, IllegalActionException {
        ...
        while (it.hasNext()) {
        	...
        	for (int i = 1; i < invocations.size(); i += 2) {
        		long time = (_modePeriod / frequency * ((Integer) invocations.get(i) - 1)) % _modePeriod;
        ...
        while (it.hasNext()) {
            ...
            long invocationPeriod = _modePeriod / TDLModeScheduler.getFrequency((NamedObj) transition);
            if (invocationPeriod % super.lcmPeriod != 0) 
                throw new TDLModeSchedulerException("Mode Switch can only take place when no task is running.");
            _scheduleActivity("calculate mode switch", transition, getFrequency(transition));
        }
        ...
        while (it.hasNext()) {
            ...
            _scheduleAtomicActivities("fast task", list, getFrequency((NamedObj) actor));
        }
    }
    
    private void _scheduleSingleActivity(String activityName, Object activity, long time) throws TDLModeSchedulerException {  
        if (_modeSchedule.get(time) == null)
            _modeSchedule.put(time, new HashMap());
        if (((HashMap)_modeSchedule.get(time)).get(activityName) == null)
            ((HashMap)_modeSchedule.get(time)).put(activityName, new ArrayList());
        ((ArrayList)((HashMap)_modeSchedule.get(time)).get(activityName)).add(activity);
    }
     ...
     * @throws TDLModeSchedulerException
     */
    private void _scheduleActivity(String activityName, Object activity, int frequency) throws TDLModeSchedulerException {  
        if ((_modePeriod % lcmPeriod) / frequency != 0)
            throw new TDLModeSchedulerException("Frequency " + frequency + " is not valid for mode period " + _modePeriod + ".");
        for (long i = 0; i < _modePeriod; i = i + _modePeriod / frequency) {
            ...
            if (((HashMap)_modeSchedule.get(i)).get(activityName) == null)
                ((HashMap)_modeSchedule.get(i)).put(activityName, new ArrayList());
            
            ((ArrayList)((HashMap)_modeSchedule.get(i)).get(activityName)).add(activity);
        }
    }
     ...
     * @throws TDLModeSchedulerException
     */
    private void _scheduleAtomicActivities(String activityName, List activities, int frequency) throws TDLModeSchedulerException {
        if ((_modePeriod % lcmPeriod) / frequency != 0)
            throw new TDLModeSchedulerException("Frequency " + frequency + " is not valid for mode period " + _modePeriod + ".");
        for (long i = 0; i < _modePeriod; i = i + _modePeriod / frequency) {
            if (_modeSchedule.get(i) == null)
            ...
            ((ArrayList)((HashMap)_modeSchedule.get(i)).get(activityName)).addAll(activities);
        }
    }
...
//        	ttModeSchedule.put(0L, null);
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < _activities.length; i++) {
                ...
                if (_modeSchedule.get(time) != null) {
                    Collection tdlActivities = (Collection)((HashMap)_modeSchedule.get(time)).get(activityName);
                    if (tdlActivities != null)
                        slotActivities.addAll(tdlActivities);
                }
                
                if (ttModeSchedule.get(time) != null) {
                    Collection ttActivities = (Collection)((HashMap)ttModeSchedule.get(time)).get(activityName);
