...
update
set savedir "[pwd]"
if {"NCCompositeActor.tcl PtinyOSActor.tcl PtinyOSDirector.tcl" != ""} {foreach i [list NCCompositeActor.tcl PtinyOSActor.tcl PtinyOSDirector.tcl] {puts $i; cd "$savedir"; if [ file exists $i ] { if [ catch {source $i} msg] {puts "\nWARNING: Sourcing $i resulted in an error,\nso we are incrementing the error count.\nThe error was: $msg\n"; incr FAILED}}}}
