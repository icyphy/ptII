            ...
            String qName, Attributes atts)
    {
        ...
        if ("".equals (uri))
        {
            if("FUZZIFY".equals(qName)||"DEFUZZIFY".equals(qName))
            {
                ...
                linguisticVarArray.add(new LinguisticVariable(atts.getValue(index)));
            }
            if("TERM".equals(qName)){
                if(startVar == true){
                    ...
                    (linguisticVarArray.get(currentIndex)).add(localName,a,b,c,d);
                }
            }
...
public class FuzzyLogic extends Transformer{
    ...
    /*  Open the file specified in the rulesFileName Parameter,
     *  create an instance of a fuzzy logic engine and populate
     * the engine based on the information provided in the rules file
     * */
    public void preinitialize() throws IllegalActionException
    {
        try{
            ...
            }else if(rulesFileName.getExpression().equalsIgnoreCase("propulsion")){
            ...
            // this may not be the ideal way to do this but lets make due with it for now
            e.printStackTrace();  
        }
    }

    /*
     *  Evaluates the Fuzzy Logic Rules specified and determines the output for this component
     * */
    public void fire() throws IllegalActionException {
        super.fire();
        if (_debugging) {
            _debug("FuzzyLogicFire function called for: "+rulesFileName.getExpression());
        }
        ...
        try{
            ...
            // here we simply assume that we need to defuzzify the last linguistic variable
            result += linguisticVarArray.get(myParser.getIndexToDefuzzify()).defuzzify();
            myCost = componentType.getExpression()+" "+rulesFileName.getExpression().substring(0, rulesFileName.getExpression().length()-4)+" cost is medium."+StringUtilities.getProperty("line.separator");;
            myRisk = componentType.getExpression()+" "+rulesFileName.getExpression().substring(0, rulesFileName.getExpression().length()-4)+" risk is medium."+StringUtilities.getProperty("line.separator");;
            myMass = componentType.getExpression()+" "+rulesFileName.getExpression().substring(0, rulesFileName.getExpression().length()-4)+" mass is medium."+StringUtilities.getProperty("line.separator");;
            if (_debugging) {
