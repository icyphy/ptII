...
fmi2Status fmi2HybridSetupExperiment(fmi2Component c, fmi2Boolean toleranceDefined, fmi2IntegerTime tolerance,
    ...
    if (invalidState(comp, "fmi2HybridSetupExperiment", MASK_fmi2SetupExperiment))
        return fmi2Error;
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2HybridSetupExperiment: toleranceDefined=%llu tolerance=%llu",
        toleranceDefined, tolerance)
...
fmi2Status fmi2SetFMUstate (fmi2Component c, fmi2FMUstate FMUstate) {
    ...
    if (NUMBER_OF_EVENT_INDICATORS > 0) {
        for (i = 0; i < NUMBER_OF_EVENT_INDICATORS; i++) {
            // dest->r[i] = source->r[i];
            dest->isPositive[i] = source->isPositive[i];
        }
    }
...
fmi2Status fmi2HybridGetMaxStepSize (fmi2Component c, fmi2IntegerTime currentCommunicationPoint, fmi2IntegerTime *value) {
    ...
    fmi2Status status = localCommunicationStepSize.status;
