...
fmi2Status fmi2HybridDoStep(fmi2Component c, fmi2IntegerTime currentCommunicationPoint,
    ...
    // *** start of fix ***
    // Comment DAVID:
    // THis is the old code. But, I think the conversion to fmi2Real must be
    // done before division.
    
    //fmi2Real hLocal = ((currentCommunicationPoint + communicationStepSize) / comp->resMagnitude - comp->time);

    // The following line shows the updated code. I think that this should
    // should be correct.
