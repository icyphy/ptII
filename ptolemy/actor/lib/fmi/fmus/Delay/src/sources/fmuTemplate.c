...
fmi2Status fmi2HybridDoStep(fmi2Component c, fmi2Integer currentCommunicationPoint,
    ...
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2HybridDoStep, event queue info: "
        "nextEventInQueue = (%u, %u), "
        "_isTime() = fmi2%s",
        _getTime(comp),
        _getIndex(comp),
        _isTime(comp) ? "True" : "False")

    if (communicationStepSize < 0) {
        FILTERED_LOG(comp, fmi2Error, LOG_ERROR,
            "fmi2HybridDoStep: communication step size must be >= 0. Fount %u.",
            communicationStepSize)
        comp->state = modelError;
        return fmi2Error;
    }

    double ei = getEventIndicator(comp);
    if (ei < 0) {
        FILTERED_LOG(comp, fmi2OK, LOG_EVENT,
            "fmi2HybridDoStep: state event at (%u, %u), detected event",
            comp->time,
            comp->microstep)
        stateEvent++;
    }
    ...
    if (comp->eventInfo.nextEventTimeDefined &&
            ((comp->time + communicationStepSize - comp->eventInfo.nextEventTime) > 0)) {
        FILTERED_LOG(comp, fmi2Discard, LOG_EVENT,
            "fmi2HybridDoStep: time event missed at (%u, %u),  it was at %u",
            comp->time,
            comp->microstep,
            comp->eventInfo.nextEventTime)
    ...
    if (comp->eventInfo.nextEventTimeDefined &&
        ...
        FILTERED_LOG(comp, fmi2OK, LOG_EVENT,
            "fmi2HybridDoStep: time event detected at (%u, %u)",
            comp->time,
            comp->microstep)
        timeEvent = 1;
    }
    ...
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL,
            "fmi2HybridDoStep: before eventUpdate at (%u, %u), "
            "_isTime() = fmi2%s, "
            "_isEmpty() = fmi2%s, "
            "inputPresent = fmi2%s ",
            comp->time, comp->microstep,
            _isTime(comp) ? "True" : "False",
            _isEmpty(comp) ? "True" : "False",
            (hr(input_) == present_) ? "True" : "False")
    ...
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL,
            "fmi2HybridDoStep, event queue info: "
            "nextEventInQueue = (%u, %u), "
            "_isTime() = fmi2%s",
            _getTime(comp),
            _getIndex(comp),
    ...
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL,
            "fmi2HybridDoStep: after eventUpdate at (%u, %u), "
            "_isTime() = fmi2%s, "
            "_isEmpty() = fmi2%s, "
            "inputPresent = fmi2%s ",
            comp->time,
...
fmi2Status fmi2HybridGetMaxStepSize (fmi2Component c, fmi2Integer *value) {
    ...
    FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2HybridGetMaxStepSize: "
        ...
        "communicationStepSize = %u, "
        "nextEventTime = %u "
        "nextEventTimeDefined = %u ",
        comp->time, comp->microstep,
        communicationStepSize,
        comp->eventInfo.nextEventTime,
        comp->eventInfo.nextEventTimeDefined)

    *value = communicationStepSize;
    return fmi2OK;
}
