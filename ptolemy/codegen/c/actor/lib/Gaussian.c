...
int Gaussian_next(int bits, double* seed) {
    *seed = (((long long) *seed * 0x5DEECE66DLL) + 0xBLL)
        & ((1LL << 48) - 1);
    return (int)((signed long long) *seed >> (48 - bits));
}
    
double Gaussian_nextDouble(double* seed) {
    return (((long long)Gaussian_next(26, seed) << 27)
            + Gaussian_next(27, seed)) / (double)(1LL << 53);
}
/**/

/*** preinitBlock ***/
    double $actorSymbol(actorSeed);
...
/*** setSeedBlock ***/
    ...
    $actorSymbol(actorSeed) = ((long long) $val(seed) ^ 0x5DEECE66DLL)  & ((1LL << 48) - 1);
/**/
...
/*** fireBlock ***/
    if ($actorSymbol(haveNextNextGaussian)) {
        ...
        do { 
            $actorSymbol(v1) = 2 * Gaussian_nextDouble(&$actorSymbol(actorSeed)) - 1;   // between -1.0 and 1.0
            $actorSymbol(v2) = 2 * Gaussian_nextDouble(&$actorSymbol(actorSeed)) - 1;   // between -1.0 and 1.0
