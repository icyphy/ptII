...
import org.terraswarm.gdp.Gdp10Library.EP_THR_MUTEX;
/**
...
public /*abstract*/ class gdp_datum extends Structure {
	...
	public gdp_datum() {
		super();
                new Exception("Calling gdp_datum()").printStackTrace();
	}
	protected List<? > getFieldOrder() {
            return Arrays.asList("mutex", "next", "inuse", "recno", "ts", "dlen", "dbuf");
	}
	/**
	...
	public gdp_datum(EP_THR_MUTEX mutex, gdp_datum.ByReference next, long recno, EP_TIME_SPEC ts, NativeSizeT dlen, PointerByReference dbuf) {
		super();
                new Exception("Calling gdp_datum(mutex ... ").printStackTrace();
		this.mutex = mutex;
		this.next = next;
		this.recno = recno;
		this.ts = ts;
	...
	public gdp_datum(Pointer peer) {
		super(peer);
                new Exception("Calling gdp_datum(" + peer + ")").printStackTrace();
	}
...
//         public ByReference() {new Exception("Calling gdp_datum ByReference()").printStackTrace();}
